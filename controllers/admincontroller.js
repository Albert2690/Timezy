    ...
    if (admin) {
      ...
      if (passwordmatch) {
        if (admin.is_admin > 0) {
          ...
          res.render("login", {
            message: "Incorrect password or invalid username  ",
          });
        }
      } else {
        res.render("login", {
...
const dashboard = async (req, res) => {
  try {
    ...
    //   if(!orders[0].totalpricesum){
    //     orders[0].totalpricesum=0
    //     return res.render('home',{salesdata,salesCount,orders,category,productCountsByCategory})
    //   }
...
const updatecategories = async (req, res) => {
  try {
    ...
    if (category) {
      ...
      res
        .send(500)
        .redirect("/editcategory", { message: "Category couldnt update" });
    }
  } catch (error) {
    console.log(error);
    res.status(500).send("Internal Server Error");
  ...
  try {
    ...
    const sales = await order
      ...
      const totalSalesCount = await order.countDocuments({
        createdAt: { $lte: postDate }, // Filter by date range
      });
    ...
    res.render("salesreport1", { sales, preDate, postDate,totalPages,page });
  } catch (error) {
    console.log(error);
    res.status(500).send("Internal server Error");
  }
