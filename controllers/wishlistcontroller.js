...
const { LEGAL_TCP_SOCKET_OPTIONS } = require('mongodb')
 ...
 const addwishlist = async (req,res)=>{
    try{
        ...
        if(existing===-1){
           const hello=  await wishlist.findOneAndUpdate({user:user._id},{$push:{items:{product:productid}}})
           if(hello){
           }
        }
 ...
 const remove = async(req,res)=>{
    try{
        const productid = req.body.productid
        const userid= req.session.user_id
        const update = await wishlist.findOneAndUpdate({user:userid},{$pull:{items:{product:productid}}})
        if(update){
