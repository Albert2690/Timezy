  ...
  try {
    ...
    if (!cartproduct) {
      await cart.findOneAndUpdate(
        ...
        {
          $push: {
            cartitems: { product: productid, quantity: quantity, total: total },
          },
        }
      );
    } else {
    ...
    if (usercart) {
      ...
      products.forEach((product) => {
        ...
        if (
          ...
          product.product.offervalidity >= Date.now()
        ) {
          itemtotal = Number(
...
const decreasequantity = async (req, res) => {
  try {
    ...
    if (
      usercart.cartitems[productindex].product.discountpercentage &&
      usercart.cartitems[productindex].product.offervalidity.getTime() >=
        Date.now()
    ) {
      usercart.subtotal -= Math.round(
        (usercart.cartitems[productindex].product.price *
          (100 - usercart.cartitems[productindex].product.discountpercentage)) /
          100
      );
      usercart.cartitems[productindex].total -= Math.round(
        (usercart.cartitems[productindex].product.price *
          (100 - usercart.cartitems[productindex].product.discountpercentage)) /
          100
      );
      ...
      usercart.cartitems[productindex].total -= Number(
        usercart.cartitems[productindex].product.price
      );
    }
    ...
    if (
      ...
      total = Math.round(
        ((usercart.cartitems[productindex].product.price *
          (100 - usercart.cartitems[productindex].product.discountpercentage)) /
          100) *
          usercart.cartitems[productindex].quantity
      );
    } else {
      total =
        usercart.cartitems[productindex].product.price *
  ...
  try {
    ...
    if (usercart) {
      ...
      if (productindex == -1) {
        return res.json({
          success: false,
          message: "cart item couldnot Found",
        });
      }
      usercart.cartitems[productindex].quantity += 1;
      if (
  ...
  try {
    ...
    if (addressofdelivery) {
      res.render("checkout1", {
        user: user,
        addresess: addressofdelivery.addresses,
        products,
        subtotal,
      });
    }
