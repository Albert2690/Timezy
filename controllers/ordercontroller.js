...
const vieworders = async (req, res) => {
    try {
        ...
        const usercart = await order.find({ user: userid }).populate('items.product')
...
const cancelorder = async (req, res) => {
    try {
        ...
        const userorder = await order.findOneAndUpdate({ _id: orderid }, { $set: { status: status } })
        if (userorder) {
            res.json({
...
const cancelstatus = async (req, res) => {
    ...
    const updateorder = await order.findOneAndUpdate({ _id: orderid }, { $set: { status: status } })
    if (status === 'cancelled') {
        ...
        try {
            ...
            const orderid = req.body.orderid
            const status = req.body.status
            const product = await order.findOne({ _id: orderid }).populate("items.product")
            if (status === 'cancelled') {
                ...
                product.items.forEach(async (items) => {
                    ...
                    const update = await Product.findOneAndUpdate({ _id: prodcutid }, { $set: { stock: prodcutstock } })
...
const returnstatus = async (req, res) => {
    try {
        ...
        const orderid = req.body.orderid
        const status = req.body.status
        const userorders = await order.findOne({ _id: orderid }).populate("items.product")

        let refundamount = 0
        refundamount += userorders.total

        if (status === "return accepted") {
            await User.findOneAndUpdate({ _id: userid }, { $set: { wallet: refundamount } })
...
const orderslist = async (req, res) => {
    try {
        const userid = req.session.user_id
        const userorder = await order.find({}).populate("items.product").populate("user")

        res.render('orderlist', { userorder })
    } catch (error) {
        console.log(error); res.status(500).send("Internal Server Error")
...
const orderstatus = async (req, res) => {
    try {
        ...
        const update = await order.findOneAndUpdate({ _id: orderid }, { $set: { status: status } })
...
module.exports = {
    confirmorder, vieworder, vieworders,
    cancelorder, orderslist, orderstatus, cancelstatus, returnstatus, verifypayment,invoice
}